---
title: "Zap Agent Framework: Eve-Powered Recipe Execution"
sidebarTitle: "Overview"
description: "Zap is an Eve app convention. Recipes are portable skill directories that coding agents read, edit, validate, and run using the same CLI commands as humans."
---
...
| `save_zap` | Save an approved `Zap.md` recipe as a packaged Eve skill (requires user approval) |
| `stitch_zap` | Stitch generated video segments into `Zap.mp4` inside the Eve sandbox using ffmpeg |
| `edit_video` | Submit a video edit or ReViz request through the Zap provider router (always requires user approval) |
| `generate_image` | Submit an image generation or image edit request through the Zap provider router |
| `generate_video` | Submit a video generation or extension request through the Zap provider router (requires approval for durations over 120 s) |
