---
title: "Zap CLI Global Flags and Output Formats"
sidebarTitle: "CLI Flags"
description: "Reference for Zap CLI flags shared across commands: --json for machine-readable output, --live for provider spend, --input for recipe inputs, and --force."
---
...
<ParamField query="--version / -v" type="boolean">
  Print the CLI version and exit. Accepts both `--version` and `-v`, or use `zap --version` as a command.
...
<ParamField query="--help / -h" type="boolean">
  Print the help text listing all commands and common flags, then exit. Also triggered by `zap help` or `zap` with no arguments.
...
| Variable | Effect |
|----------|--------|
| `ZAP_PROVIDER=mock` | Override provider selection to `mock` at runtime |
| `ZAP_LINT_ALLOW_LIVE_DEFAULT=1` | Silence the `zap lint` warning about non-mock `defaults.provider` |

These can be set in your `.env` or `.env.local` file (automatically loaded by the CLI on startup) or exported in your shell:
